...
export function CityPlacesView({
  places,
  city,
  state,
  introText,
  ...
  const initialReligion = searchParams.get('religion')?.trim() || '';

  const initialSelected: Record<string, string[]> = initialReligion
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div className="mb-6 flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <a href="/locations" className="hover:text-primary-600">Locations</a>
        <span>/</span>
        <span className="font-medium text-slate-900">{city}, {state}</span>
      </div>

      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 md:text-4xl">
            <span className="text-primary-600">{filtered.length}</span>{' '}
            {selected.religion?.length === 1
              ? `${selected.religion[0]} places of worship in ${city}, ${state}`
              : `Places of Worship in ${city}, ${state}`}
          </h1>
